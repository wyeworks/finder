...
RSpec.describe GroupsController, type: :request do
  ...
  # Destroy Member
  describe 'DELETE /groups/:group_id/members/:user_id' do
    let(:group) { create :group }
    ...
    before do
      group.members.create(user: admin, role: 'admin')
      group.members.create(user: member, role: 'participant')
      group.members.create(user: another_member, role: 'participant')
    end

    context "when user is the group's admin" do
      before do
        delete "/groups/#{group.id}/members/#{member.id}", headers: headersAdmin
      end

      it 'deletes the member successfully' do
        expect(response).to have_http_status(:no_content)
        group.reload
        expect(group.members.find_by(user: member)).to be_nil
      end
    end
    ...
    context "when user is a group's member" do
      context 'and tries to remove himself' do
        before do
          delete "/groups/#{group.id}/members/#{member.id}", headers: headersMember
        end
      ...
      context 'and tries to remove another member' do
        before do
          delete "/groups/#{group.id}/members/#{another_member.id}", headers:
        end
        ...
        it 'does not delete the other member' do
          expect(group.members.find_by(user: another_member)).not_to be_nil
        end
      end
      ...
      context 'when member destruction fails' do
        ...
        before do
          ...
          delete "/groups/#{group.id}/members/#{member.id}", headers:
        end
